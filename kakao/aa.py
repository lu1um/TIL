...
def Me(opp, turn, opp_prev, opp_last_pattern):
    like = [0, 0, 0]
    for opp_pattern in opp_last_pattern[:140]:
        thisTurn = opp_pattern[turn]
        if thisTurn == -1:
            break
        like[thisTurn] += 1
    target = RABBIT
    max_like = 0
    for animal in [DEER, RABBIT, SNAKE]:
        if like[animal] > max_like:
            target = animal
            max_like = like[animal]
...
for i in range(1, f_inx):
    for j in range(f_inx):
        ...
        for k in range(10):
            ...
            if a == DEER and b == DEER:
                a_score = 50; b_score = 50;
            elif a == DEER and b == RABBIT:
                a_score = 0; b_score = 20;
            elif a == DEER and b == SNAKE:
                a_score = 0; b_score = 10;
            elif a == RABBIT and b == DEER:
                a_score = 20; b_score = 0;
            elif a == RABBIT and b == RABBIT:
                a_score = 20; b_score = 20;
            elif a == RABBIT and b == SNAKE:
                a_score = 0; b_score = 30;
            elif a == SNAKE and b == DEER:
                a_score = 10; b_score = 0;
            elif a == SNAKE and b == RABBIT:
                a_score = 30; b_score = 0;
            elif a == SNAKE and b == SNAKE:
                a_score = 10; b_score = 10;
            ...
            print(
                f"Turn [{k + 1}] [{names[team_a]}:({a_str})] vs [{names[team_b]}:({b_str})] ---> score [{a_game_score}] / [{b_game_score}] ")
